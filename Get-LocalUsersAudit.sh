...
# RunId:
# - If $1 is provided (plain or JSON), use it
# - Else: default to UTC YYYYMMDD (e.g. 20251217)
arg1="${1:-}"
run_id=""
...
OUT_FILE="${OUT_DIR}/${run_id}_LocalUsersData.csv"
...
# sudoers parsing (best effort)
sudo_files=(/etc/sudoers)
if [ -d /etc/sudoers.d ]; then
  while IFS= read -r -d '' f; do sudo_files+=("$f"); done < <(find /etc/sudoers.d -maxdepth 1 -type f -print0 2>/dev/null || true)
fi

sudo_groups=(); sudo_users=()
for f in "${sudo_files[@]}"; do
  ...
  while IFS= read -r line; do
    ...
    case "$first" in Defaults|User_Alias|Host_Alias|Cmnd_Alias|Runas_Alias|#include|#includedir|Include|includedir) continue ;; esac
    if [[ "$first" == %* ]]; then sudo_groups+=("${first#%}")
    elif [[ "$first" == "+"* ]]; then continue
    else sudo_users+=("$first"); fi
  done < "$f"
done
...
while IFS=: read -r user _ uid _ _ _ shell; do
  ...
  sudo_effective="No"; sudo_reason=""
  if command -v sudo >/dev/null 2>&1 && [[ "${EUID:-9999}" -eq 0 ]]; then
    out="$(sudo -l -U "$user" 2>/dev/null || true)"
    if grep -qiE 'may run the following commands|may run the following' <<<"$out"; then
      sudo_effective="Yes"; sudo_reason="sudo -l -U evaluation"
    fi
  else
    if printf "%s\n" "${sudo_users[@]}" | grep -qx "$user" 2>/dev/null; then
      ...
      for g in "${sudo_groups[@]}"; do
        [[ -z "$g" ]] && continue
        if [[ ";$groups;" == *";$g;"* ]]; then sudo_effective="Yes"; sudo_reason="member of sudoers group: $g"; break; fi
      done
    fi
  fi
